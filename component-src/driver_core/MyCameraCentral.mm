...
#define FAKE_CAMERA_NAME		@"SharedDriver"
#else
#define FAKE_CAMERA_NAME		@"Processing"
#endif
...
- (void) listAllCameras
{
    ...
    for (i = 0; i < (int) [cameraTypes count]; i++) 
    {
...
- (void) listAllDuplicates
{
    ...
    for (i = 0; i < (int) [cameraTypes count]; i++) 
    {
        ...
        for (j = 0; j < (int) [cameraTypes count]; j++) 
        {
...
- (BOOL) startupWithNotificationsOnMainThread:(BOOL)nomt recognizeLaterPlugins:(BOOL)rlp{
	...
	// ROGER
    [self registerCameraDriver:[Processing class]];
    [self registerCameraDriver:[SharedDriver class]];
    ...
    for (i=0;i<(int)[cameraTypes count];i++) {
		...
		// ROGER
		//NSLog(@"CAMERA [%d] name[%@] cid[%d]",i,[info cameraName],[self idOfCameraWithIndex:(short)i]);
		...
		// ROGER
		if ([[info cameraName] isEqualToString:FAKE_CAMERA_NAME])
		{
			NSLog(@"FAKE CAMERA starting... name[%@]",FAKE_CAMERA_NAME);
			ret = -1;
			//Put the new entry to the list of available cameras
			[cameras addObject:info];
			//Spread the news that a camera was plugged in
			[self cameraDetected:[info cid]];
		}
		else
		// continue...
...
// ROGER
//
- (void) startFakeCamera {
	int i;
    for (i=0;i<(int)[cameraTypes count];i++)
	{
        MyCameraInfo* info=[cameraTypes objectAtIndex:i];
		if ([[info cameraName] isEqualToString:FAKE_CAMERA_NAME])
		{
			NSLog(@"FAKE CAMERA starting... cid[%d] name[%@]",[info cid],FAKE_CAMERA_NAME);
			//Put the new entry to the list of available cameras
			[cameras addObject:info];
			// force detection
			[self cameraDetected:[info cid]];
...
- (void) shutdown {
    ...
    while ([cameraTypes count]>0) {
		...
		// ROGER: evita crash: message release sent to freed object
        //[info release];
    }
    [pool release];
}
...
- (CameraError) useCameraWithID:(unsigned long)cid to:(MyCameraDriver**)outCam acceptDummy:(BOOL)acceptDummy {
    ...
    for (l=0;(l<(int)[cameras count])&&(dev==NULL);l++) {
...
- (NSString *) nameForID:(unsigned long) cid 
{
    ...
    for (l = 0; l < (int) [cameras count]; l++) 
        if ([[cameras objectAtIndex:l] cid] == cid) 
        {
 			...
 			for (i = 0; i < (int) [cameras count]; i++)  // look again over all cameras
            {
...
- (BOOL) setCameraToDefaults:(MyCameraDriver*) cam {
    ...
    if (ok) {
        ...
        if ([camDict objectForKey:@"resolution"]&&[camDict objectForKey:@"fps"])
            [cam setResolution:(CameraResolution)[[camDict objectForKey:@"resolution"] shortValue] fps:[[camDict objectForKey:@"fps"] shortValue]];
